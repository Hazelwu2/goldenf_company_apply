...
- 切換幣別時提醒並移除不相容的產品商。
- MA／SMA 的後台白名單與聯絡 Email 各有獨立的「與 A 相同」選項，可只沿用其中一項。
- IPv4、IPv6、CIDR 白名單格式檢查，格式錯誤的項目會標示出來。
...
- `components` 是「可重複拼裝的小零件」，例如欄位標籤、多筆輸入框、驗證碼與階層圖。
...
| MA／SMA 代碼 | 2–12 碼英數字，自動轉大寫 |
| 後台帳號 | 6–10 碼小寫英數字 |
| 白名單 | 至少一筆合法 IPv4 或 IPv6，可附 CIDR；以陣列保存 |
| 聯絡 Email | 選填；可填多筆，每筆都必須符合 Email 格式；以陣列保存 |
| 營運商產品商 | 至少選一個，且必須支援目前幣別 |
| 營運市場 | 至少選一個；送出值為 ISO 3166-1 alpha-2 代碼 |
| 站台狀態 | 必選 |
| 站台網址／測試帳號／測試密碼 | 已有網站時三者必填，網址須為 http／https 完整網址；尚在開發中時三者必須留空 |
| 通訊軟體 | 僅 A 需填，必填；送出值為 `Teams` 或 `telegram` |
| 通訊群組 | 僅 A 需填，必填；Telegram 或 Teams 的群組名稱 |
...
後台白名單、API 白名單、聯絡 Email 與營運市場在 Store 中都以 `string[]` 保存，形狀與 Create API 預期的陣列一致，串接時可直接送出，不需要再做字串切割：

```ts
boWhitelist: ['203.0.113.10', '198.51.100.0/24']
apiWhitelist: ['203.0.113.10']
emails: ['ops@example.com', 'risk@example.com']
operatingMarkets: ['CN', 'VN']   // ISO 3166-1 alpha-2
```

伺服器端仍須重新驗證每一筆項目，不能相信前端已過濾。
